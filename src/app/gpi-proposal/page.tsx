...
export default function GPIProposalApp() {
  ...
  <Card className="p-6"><h3 className="text-xl font-bold text-white">District Procurement Progress</h3><p className="mt-2 text-sm text-slate-400">Click a bar to inspect district details.</p><div className="mt-6 h-72"><ResponsiveContainer width="100%" height="100%"><BarChart data={districtData} onClick={(s) => { const payload = s?.activePayload?.[0]?.payload as (typeof districtData)[number] | undefined; if (payload) setActiveDistrict(payload); }}><CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.08)" /><XAxis dataKey="district" tick={{ fill: "#cbd5e1", fontSize: 11 }} /><YAxis tick={{ fill: "#cbd5e1", fontSize: 11 }} /><Tooltip /><Bar dataKey="procured" fill="#34d399" /><Bar dataKey="payments" fill="#38bdf8" /></BarChart></ResponsiveContainer></div></Card>
