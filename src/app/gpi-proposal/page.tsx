...
export default function GPIProposalApp() {
  ...
  <Card className="p-6"><h3 className="text-xl font-bold text-white">Payment Time Improvement</h3><div className="mt-6 h-72"><ResponsiveContainer width="100%" height="100%"><AreaChart data={paymentTrend}><CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.08)" /><XAxis dataKey="day" tick={{ fill: "#cbd5e1", fontSize: 11 }} /><YAxis tick={{ fill: "#cbd5e1", fontSize: 11 }} /><Tooltip /><Area type="monotone" dataKey="manual" stroke="#f59e0b" fill="#f59e0b33" onClick={(entry) => { const payload = (entry as { payload?: (typeof paymentTrend)[number] }).payload; if (payload) setPaymentPoint(payload); }} /><Area type="monotone" dataKey="digital" stroke="#22c55e" fill="#22c55e33" onClick={(entry) => { const payload = (entry as { payload?: (typeof paymentTrend)[number] }).payload; if (payload) setPaymentPoint(payload); }} /></AreaChart></ResponsiveContainer></div><p className="mt-3 text-sm text-slate-300">Selected {paymentPoint.day}: digital saves <b>{paymentPoint.manual - paymentPoint.digital}h</b> vs manual.</p></Card>
